...
import 'package:fittrack/logic/bloc/group_training/group_event.dart';
import 'package:fittrack/logic/bloc/group_training/group_state.dart';

import '../screens/features/group_trainings/bloc/group_training_bloc.dart';
import '../screens/features/group_trainings/bloc/group_training_event.dart';

class GroupTrainingWidgets extends StatelessWidget {
  final GroupTraining training;
  final ConfirmationDialog _confirmationDialog = ConfirmationDialog();
  final ErrorDialog _errorDialog = ErrorDialog();

  GroupTrainingWidgets({super.key, required this.training});
  ...
  void _handleTrainingRegistration(BuildContext context) async {
    // Check if there are available spots
    if (training.registrationsRemaining <= 0) {
      _errorDialog.showErrorDialog(
          context,
          'Немає вільних місць',
    ...
    bool confirmed = await _confirmationDialog.showConfirmationDialog(
        context,
        'Запис на тренування',
        'Бажаєте записатися на групове тренування "${training.description}" о ${DateFormat.Hm().format(training.dateTime)}?'
    );

    if (confirmed) {
      // Add user to training using bloc
      context.read<GroupBloc>().add(AddUserToGroupTraining(trainingId: training.id));
    }
  }
  ...
  Widget build(BuildContext context) {
    return BlocListener<GroupBloc, GroupState>(
      listener: (context, state) {
        if (state is GroupLoaded) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                'Вас успішно записано на тренування',
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: const Text(
                'Помилка при записі на тренування',
              ),
              behavior: SnackBarBehavior.floating,
              margin: const EdgeInsets.all(16),
              elevation: 6,
      ...
      child: InkWell(
        onTap: () => _handleTrainingRegistration(context),
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          ...
                          const SizedBox(height: 8),
                          Text(
                            "Вільних місць: ${training.registrationsRemaining.toString()}",
                            style: Theme.of(context).textTheme.headlineMedium?.copyWith(
                              color: training.registrationsRemaining > 0
